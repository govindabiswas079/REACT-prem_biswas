...
const WorkExperiance = ({ mode }) => {
  const { GetExperiance, GetMonths } = useSelector(state => state?.reducerSlicer)
  ...
  return (
    <Box id={'WorkExperiance'} sx={{ marginTop: 10 }}>
      ...
      <Container>
        {GetExperiance?.length ?
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: "center" }}>
            <Typography variant={'h4'} sx={{ textAlign: 'center', textTransform: 'uppercase', pt: 1, pb: 2.5, fontFamily: 'Nunito', fontWeight: '700' }}>Experiance</Typography>
          </Box> : null}
        <Grid container justifyContent="center" spacing={1}>
          {GetExperiance?.map((value, index) => (
            <Grid key={index} item xs={12} sm={12} md={6} lg={6}>
              <Box sx={{ /* height: 210, */ width: '100%', transition: '0.3s ease-in-out', boxShadow: '0 4px 44px rgb(17 29 48 / 10%)', backgroundColor: mode ? '#132f4c' : '#FFFFFF', borderRadius: 2, borderWidth: 2, borderColor: mode ? '#132f4c' : '#FFF', borderStyle: 'solid', '&:hover': { boxShadow: '0 4px 44px rgb(17 29 48 / 45%)', transition: '0.3s ease-in-out', backgroundColor: mode ? '#132f4c' : '#FFFFFF', borderWidth: 2, borderColor: mode ? '#1e4976' : 'blue', borderStyle: 'solid' } }}>
                <Box sx={{
                  display: 'flex',
                  alignItems: 'center',
                ...
                <Box sx={{ padding: 1 }}>
                  <Typography sx={{ fontFamily: 'Open Sans', fontWeight: 700, fontSize: 18, color: mode ? 'rgb(208 223 255/1)' : '#252F40' }}>{value?.position}</Typography>
                  <Typography sx={{ paddingTop: 0, fontFamily: 'Nunito', fontWeight: 500, fontSize: 14, color: mode ? 'rgb(163 179 188/1)' : 'rgb(91 100 120/1)' }}>{value?.type}</Typography>
                  ...
                  <Typography sx={{ paddingTop: 2, fontFamily: 'Open Sans', fontWeight: 700, fontSize: 16, color: mode ? 'rgb(208 223 255/1)' : '#252F40' }}>{value?.company}</Typography>
