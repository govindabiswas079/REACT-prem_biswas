        ...
        <Grid container justifyContent="center" spacing={2}>
          {data.map((value, index) => (
            <Grid key={index} item xs={6} sm={4} md={2} lg={2}>
              <Box data-aos='zoom-in' data-aos-offset='200' sx={{ height: 140, width: '100%', boxShadow: '0 4px 44px rgb(17 29 48 / 10%)', transition: '0.3s ease-in-out', display: 'flex', justifyContent: 'center', backgroundColor: mode ? '#132f4c' : '#FFFFFF', borderRadius: 2, borderWidth: 2, borderColor: mode ? '#132f4c' : '#FFF', borderStyle: 'solid', '&:hover': { boxShadow: '0 4px 44px rgb(17 29 48 / 45%)', color: 'blue', transition: '0.3s ease-in-out', backgroundColor: mode ? '#132f4c' : '#FFFFFF', borderWidth: 2, borderColor: mode ? '#1e4976' : 'blue', borderStyle: 'solid' } }}>
                <Box sx={{
                  display: 'flex',
